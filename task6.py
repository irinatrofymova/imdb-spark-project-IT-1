...
series_df=tabl6_tb_df.join(tabl6_te_df, tabl6_tb_df.tconst==tabl6_te_df.parentTconst, "inner") \
    .select(tabl6_tb_df.primaryTitle, tabl6_tb_df.titleType, tabl6_te_df.episodeNumber) \
    .filter(f.col('episodeNumber')>0)
#series_df.show()

rez6_df = series_df.groupBy('primaryTitle').agg({'*': 'count', 'episodeNumber': 'max'}).orderBy('max(episodeNumber)', ascending=False)
rez6_df.show(50)
